...
class FeedAcceptanceTests: XCTestCase {
    ...
    func test_onLaunch_displaysCachedRemoteFeedWhenCustomerHasNoConnectivity() {
        let sharedStore = InMemoryFeedStore()
    ...
    private func launch(
        ...
        let sut = SceneDelegate(httpClient: httpClient, store: store, scheduler: AnyDispatchQueueScheduler.immediateOnMainQueue)
    ...
    private func enterBackground(with store: InMemoryFeedStore) {
        let sut = SceneDelegate(httpClient: HTTPClientStub.offline, store: store, scheduler: AnyDispatchQueueScheduler.immediateOnMainQueue)
    ...
    private func makeData(for url: URL) -> Data {
        ...
        case "/essential-feed/v1/feed" where url.query?.contains("after_id=166FCDD7-C9F4-420A-B2D6-CE2EAFA3D82F") == true:
            return makeLastPageEmptyFeedData()
    ...
    private func makeLastPageEmptyFeedData() -> Data {
        let emptyItems: [Any] = []
        return try! JSONSerialization.data(withJSONObject: ["items": emptyItems])
    }
